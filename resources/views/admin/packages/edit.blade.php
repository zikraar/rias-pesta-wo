...
@section('page-subtitle', 'Perbarui informasi paket')
        ...
        <form method="POST" action="{{ route('admin.packages.update', $package) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div>
                ...
                <select name="category" required
                        class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300">
                    <option value="dekorasi" {{ old('category',$package->category)=='dekorasi' ? 'selected' : '' }}>Dekorasi</option>
                    <option value="rias"     {{ old('category',$package->category)=='rias'     ? 'selected' : '' }}>Rias Pengantin</option>
                    <option value="paket"    {{ old('category',$package->category)=='paket'    ? 'selected' : '' }}>Paket Lengkap</option>
                    <option value="catering" {{ old('category',$package->category)=='catering' ? 'selected' : '' }}>Catering</option>
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          class="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-300">{{ old('description', $package->description) }}</textarea>
            </div>
